...
(() => {
  ...
  function bindAppEvents(app) {
    ...
    function renderPaymentSummary() {
      ...
      elements.accountSupportSummary.textContent = hasSupported ? `????????? ????????? ${support.lastDonationAt ? new Date(support.lastDonationAt).toLocaleDateString("ru-RU") : "??? ????????"}. ?????? ??? ????? ???????? ?????? ????? ?????????? ???????? YooKassa.` : "?????? ???????? ?? ?????????? ??????? YooKassa. ?????? ????? ?? ??????????? ? ??????????.";
      if ((latestDonation == null ? void 0 : latestDonation.status) === "pending") {
        elements.accountSupportMessage.textContent = "???? ????????????? ??????. ????? ????????????? YooKassa ?????? ????????? ?????????????.";
